...
Fmsy<-function(stk,eql,nits=100,fmsy=benchmark(stk)["fmsy"],maxYear=dims(stk)$maxyear+25,f.cv=0.2,bnd=NA){
        set.seed(8778)
...
Let's visualize the $F_{MSY}$ projections:
...
```{r findings}
# Compare $F_{MSY}$ values
cat("$F_{MSY}$ Comparison:\n")
cat("Beverton-Holt $F_{MSY}$:", round(bh_refpts["msy", "harvest"], 3), "\n")
cat("Ricker $F_{MSY}$:", round(rk_refpts["msy", "harvest"], 3), "\n")
